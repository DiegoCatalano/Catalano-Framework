    ...
    public ImageHistogram ProcessImage(FastBitmap fastBitmap){
        ...
        for (int x = 1; x < height; x++) {
            for (int y = 1; y < width; y++) {
                ...
                sum = 0;
                if (gray < fastBitmap.getGray(x - 1, y - 1))    sum += 1;
                if (gray < fastBitmap.getGray(x - 1, y))        sum += 2;
                if (gray < fastBitmap.getGray(x - 1, y + 1))    sum += 4;
                if (gray < fastBitmap.getGray(x, y + 1))        sum += 8;
                if (gray < fastBitmap.getGray(x + 1, y + 1))    sum += 16;
                if (gray < fastBitmap.getGray(x + 1, y))        sum += 32;
                if (gray < fastBitmap.getGray(x + 1, y - 1))    sum += 64;
                if (gray < fastBitmap.getGray(x, y - 1))        sum += 128;
                g[sum]++;
            }
        }

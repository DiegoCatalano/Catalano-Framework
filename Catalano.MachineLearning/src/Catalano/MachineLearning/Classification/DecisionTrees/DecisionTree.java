...
public class DecisionTree implements IClassifier {
    ...
    public int Predict(double[] feature) {
        if(!buildModel)
            ...
            BuildModel(input, output, J, null, null, rule);
        return root.predict(feature);
    }
    ...
    private void BuildModel(double[][] x, int[] y, int J, int[] samples, int[][] order, SplitRule rule){
        this.buildModel = true;
    ...
    DecisionTree(DecisionVariable[] attributes, double[][] x, int[] y, int J, int[] samples, int[][] order, SplitRule rule) {
        ...
        BuildModel(x, y, J, samples, order, rule);
    }
